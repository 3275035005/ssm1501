    ...
    <div style="width: 100%; height: 100vh; overflow: hidden" class="bgImg">
      <div style="width: 500px; height: 350px; background-color:rgba(9, 50, 113 ,0.2); border-radius: 10px;
        ...
        <div style="width: 260px; margin: 0 auto; padding-top: 30px">
          <div class="item" style="font-size: 20px; color: white">垃圾分类系统登录</div>
          <div class="item">
    ...
    <el-dialog title="忘记密码" :visible.sync="dialogFormVisible" width="30%"
      ...
      <el-form :model="reset">
        ...
        <el-form-item label="姓名" label-width="150px">
          <el-input v-model="reset.name" autocomplete="off" style="width: 80%"></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    login() {
      ...
      API.post("/user/login", this.user).then(res => {
        if (res.code === 200) {
          this.$message({
            type: "success",
            message: "登录成功"
          })
          sessionStorage.setItem("user", JSON.stringify(res.row))
          // 重置路由
          // resetRouter(JSON.parse(JSON.stringify()))
