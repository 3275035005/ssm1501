    ...
    <div style="width: 100%; height: 100vh; overflow: hidden" class="bgImg">
      <div style="width: 500px; height: 420px; background-color:rgba(9, 50, 113 ,0.2); border-radius: 10px;
        ...
        <div style="width: 260px; margin: 0 auto; padding-top: 30px">
          ...
          <div class="item">
            <button style="   background: forestgreen;  border-radius: 5px; color: white; width: 100%; padding: 10px; cursor: pointer" @click="register">注册</button>
          </div>
...
export default {
  ...
  methods: {
    register() {
      ...
      API.post("/user/register", this.user).then(res => {
        if (res.code === 200) {
          this.$message({
            type: "success",
            message: "注册成功"
